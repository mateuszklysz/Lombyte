...
def fix_unit(unit: dict[str, Any]):
    ...
    text_section = next(
        section for section in unit["sections"] if section["name"] == ".text"
    )
...
def fix_report(report_path: Path):
    ...
    report["measures"]["fuzzy_match_percent"] = (
        100.0 * computed_matched_code / computed_total_code
    )
    report["measures"]["matched_code"] = str(computed_matched_code)
    report["measures"]["matched_code_percent"] = report["measures"][
        "fuzzy_match_percent"
    ]
    report["measures"]["matched_functions"] = computed_matched_functions
    report["measures"]["matched_functions_percent"] = (
        100.0 * computed_matched_functions / computed_total_functions
    )
    ...
    categories[0]["measures"]["fuzzy_match_percent"] = report["measures"][
        "fuzzy_match_percent"
    ]
    categories[0]["measures"]["matched_code"] = report["measures"]["matched_code"]
    categories[0]["measures"]["matched_code_percent"] = report["measures"][
        "matched_code_percent"
    ]
    categories[0]["measures"]["matched_functions"] = report["measures"][
        "matched_functions"
    ]
    categories[0]["measures"]["matched_functions_percent"] = report["measures"][
...
def main():
    ...
    parser.add_argument(
        ...
        metavar="report-path",
        type=Path,
        help="path to the report generated by objdiff",
    )
