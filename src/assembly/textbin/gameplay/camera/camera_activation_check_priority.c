...
#include "types.h"
struct Camera {
    u8 pad0[0x74]; s32 kind; s32 pad78; u8 priority; u8 enabled; s16 locked; u8 pad80[4];
    s16 spline; s16 target; u8 pad88[4]; s16 mode;
};
struct CamMode { s32 (*update)(struct Camera *); s32 (*check)(struct Camera *, struct Camera *); u8 pad8[0xC]; };
...
extern struct SplineEntry *D_0015EF90;
...
s32 FUN_001ec210(struct Camera *c, struct Camera *other) {
    ...
    case 0:
        ...
        return other->priority < c->priority;
    case 4:
        t = D_0015EF90[c->spline].target;
        if (other != 0 && other->locked == 0 && !(other->priority < c->priority)) {
            return 0;
        }
        return func_00214720(D_0013F3D0, t->node) != 0;
    case 7:
        if (c->target != D_0013F350.target) {
            return 0;
        }
        if (other->locked == 0 && !(other->priority < c->priority)) {
            return 0;
        }
        if (c->target != 3) {
            return 1;
